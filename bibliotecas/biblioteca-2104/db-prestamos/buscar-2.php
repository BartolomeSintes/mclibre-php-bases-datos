...
if ($prestado_1 != "" && $prestado_2 != "") {
    $consultaPrestado          = "AND $cfg[tablaPrestamos].prestado BETWEEN :prestado_1 AND :prestado_2 ";
    $parametros[":prestado_1"] = $prestado_1;
    $parametros[":prestado_2"] = $prestado_2;
} elseif ($prestado_1 != "") {
    $consultaPrestado          = "AND $cfg[tablaPrestamos].prestado >= :prestado_1 ";
    $parametros[":prestado_1"] = $prestado_1;
} elseif ($prestado_2 != "") {
    $consultaPrestado          = "AND $cfg[tablaPrestamos].prestado <= :prestado_2 ";
    $parametros[":prestado_2"] = $prestado_2;
}

if ($devuelto_1 != "" && $devuelto_2 != "") {
    $consultaDevuelto          = "AND $cfg[tablaPrestamos].devuelto BETWEEN :devuelto_1 AND :devuelto_2 ";
    $parametros[":devuelto_1"] = $devuelto_1;
    $parametros[":devuelto_2"] = $devuelto_2;
} elseif ($devuelto_1 != "") {
    $consultaDevuelto          = "AND $cfg[tablaPrestamos].devuelto >= :devuelto_1 ";
    $parametros[":devuelto_1"] = $devuelto_1;
} elseif ($devuelto_2 != "") {
    $consultaDevuelto          = "AND $cfg[tablaPrestamos].devuelto <= :devuelto_2 ";
    $parametros[":devuelto_2"] = $devuelto_2;
}
...
$consulta = "SELECT COUNT(*)
    ...
    AND $cfg[tablaPersonas].nombre LIKE :nombre
    AND $cfg[tablaPersonas].apellidos LIKE :apellidos
    AND $cfg[tablaObras].autor LIKE :autor
    AND $cfg[tablaObras].titulo LIKE :titulo "
    . $consultaPrestado
    . $consultaDevuelto;
